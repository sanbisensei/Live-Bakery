...
export default function CakeDetailPage({ params }: Props) {
  ...
  // Price including the selected size's add-on
  // const priceWithSize = cake.base_price + (selectedSize?.price_add ?? 0);
  const priceWithSize = selectedSize?.price_add ?? cake.base_price;

  const discountedPrice =
    cake.discount_pct > 0
      ? Math.round(priceWithSize * (1 - cake.discount_pct / 100))
      : null;
