...
// Add your real photo paths here — any number works, they'll crossfade in order
// every few seconds. Drop the files in /public/hero/ (or wherever) and list them,
// e.g. "/hero/cake-1.jpg". Leave empty and the placeholder mock below still shows.
const DEFAULT_HERO_IMAGES: string[] = [
  "/images/hero%20(1).jpeg",
  "/images/hero%20(2).jpeg",
  "/images/hero%20(3).jpeg",
  "/images/hero%20(4).jpeg",
  "/images/hero%20(5).jpeg",
];

function useImageRotator(count: number, intervalMs = 4000) {
...
export default function Hero({
  ...
  const activeIndex = useImageRotator(images.length, 4000);
    ...
    <section className="relative overflow-hidden px-4 md:px-10 py-14 md:py-20 bg-cream">
      ...
      <div className="relative grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-14 items-center max-w-7xl mx-auto w-full">
        ...
        {/* image panel: curtain-reveal on load, tilts toward the cursor for depth,
            and crossfades through `images` if you pass more than one. */}
        <div
          ref={wrapRef}
          className="relative"
        ...
        >
          ...
          <div className="reveal-mask">
            ...
            >
              {images.length > 0 ? (
                // crossfading photo stack — each photo sits absolutely stacked on
                // top of the others; only the active one is faded to full opacity
                images.map((src, i) => (
                  <Image
                    key={src}
                    src={src}
                    alt="Freshly baked cake from Afsana's Kitchen"
                    fill
                    priority={i === 0}
                ...
                {images.map((_, i) => (
                  <span
                    ...
                    className={`h-1.5 rounded-full transition-all duration-500 ${
                      i === activeIndex ? "w-5 bg-cream" : "w-1.5 bg-cream/60"
                    }`}
                  />
                ))}
              </div>
